...
\usepackage{subfigure}            %zwei Figures nebeneinander
\usepackage{color}
...
$P_t(1+\tau_{C,t})C_t + B_{G,t} + MB_{t+1} = (1-\tau_{N,t})W_tN_t + (1+i_{t-1})B_{G,t-1} + MB-T_t + \Gamma_t, \vspace{0.2cm}$
...
First, we focus on the impulse response of a large negative taste shock and a positive government spending shock to counteract the fall in output. The main steps to replicate the New Keynesian model and the results of \citet{Erceg.2014} are the following:
\begin{enumerate}
